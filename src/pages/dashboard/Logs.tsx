...
const Logs = () => {
  ...
  const filtered = useMemo(() => {
    let list = LOGS;
    if (focusEvent) list = list.filter((l) => l.id === focusEvent);
    if (q) list = list.filter((l) => [l.player, l.id, l.detail, l.type].some((v) => v.toLowerCase().includes(q.toLowerCase())));
    return list;
  }, [focusEvent, q]);
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Activity breakdown by event type {focusEvent && `· focused on ${focusEvent}`}</p>
      </div>

      <div className="glass rounded-xl p-4">
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by player, event ID, license, or details..." />
      </div>
      ...
      <div className="glass rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{filtered.length} of {LOGS.length} logs · time range and chart coming in Phase 3</p>
          </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {filtered.slice(0, 80).map((l, i) => (
                <tr key={l.id} className="border-b border-border/30 hover:bg-muted/20 animate-fade-up" style={{ animationDelay: `${i * 15}ms` }}>
